...
@Composable
fun EditTherapyScreen(
    personId: String,
    therapyId: String,
    onNavigateBack: () -> Unit,
    viewModel: TherapyViewModel = hiltViewModel(),
) {
    LaunchedEffect(personId, therapyId) {
        viewModel.loadTherapyForEdit(personId, therapyId)
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(if (form.step == 1) "Modifica terapia" else "Farmaci") },
                navigationIcon = {
                    IconButton(onClick = {
                        if (form.step > 1) {
                            viewModel.prevStep()
        ...
        Column(
            ...
            when (form.step) {
                1 -> EditStepOneContent(form, viewModel)
                2 -> EditStepTwoContent(form, viewModel, onNavigateBack)
            }
        }
    }
}
...
@Composable
private fun EditStepOneContent(
...
@Composable
private fun EditStepTwoContent(
    form: AddTherapyFormState,
    viewModel: TherapyViewModel,
    onDone: () -> Unit,
) {
    Column(
        ...
        Button(
            onClick = { viewModel.submitEditTherapy(onDone) },
            modifier = Modifier.fillMaxWidth(),
...
@Composable
private fun EditMedicationFormItem(
