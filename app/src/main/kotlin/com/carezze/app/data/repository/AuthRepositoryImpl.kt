...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun createUserWithEmail(email: String, password: String): Result<User> = runCatching {
        firebaseAuth.createUserWithEmailAndPassword(email, password).await().user?.toDomain()
            ?: error("Create user returned null user")
    }

    override suspend fun linkWithEmail(email: String, password: String): Result<User> = runCatching {
    ...
    override suspend fun linkWithGoogle(idToken: String): Result<User> = runCatching {
        val credential = GoogleAuthProvider.getCredential(idToken, null)
        firebaseAuth.currentUser?.linkWithCredential(credential)?.await()?.user?.toDomain()
