...
class MedicationLogRepositoryImpl @Inject constructor(
    ...
    private fun logsCollection(personId: String, therapyId: String) =
        firestore.collection("persons").document(personId)
            .collection("therapies").document(therapyId)
            .collection("medicationLogs")

    override fun observeLogs(personId: String, therapyId: String): Flow<List<MedicationLog>> = callbackFlow {
        val listener = logsCollection(personId, therapyId)
    ...
    override suspend fun logMedication(
        ...
        userId: String,
    ): Result<MedicationLog> = runCatching {
    ...
    private fun com.google.firebase.firestore.DocumentSnapshot.toDomain(therapyId: String): MedicationLog? {
        ...
        val status = getString("status")?.let { runCatching { MedicationStatus.valueOf(it) }.getOrNull() }
            ?: return null
